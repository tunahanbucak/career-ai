  ...
  return (
    ...
    >
            ...
            {breadcrumbs.map((item, index) => {
                ...
                return (
                    <div key={item.href} className="flex items-center gap-2">
                        {index > 0 && <Slash size={14} className="text-muted-foreground/60" />}
                        <Link 
                            href={item.href} 
                            className={`flex items-center gap-2 transition-colors ${isLast ? "text-foreground font-medium pointer-events-none" : "hover:text-primary"}`}
                        >
                            {Icon && <Icon size={16} />}
      ...
      <div className="flex items-center gap-3 sm:gap-4 pl-4">
            {/* Notification */}
            <Button variant="ghost" size="icon" className="relative h-10 w-10 text-muted-foreground hover:text-foreground hover:bg-secondary/80 rounded-xl transition-all">
                <Bell size={20} />
                ...
                <DropdownMenuContent align="end" className="w-56 bg-background/95 backdrop-blur-xl border-border text-foreground mt-2 px-1 py-1.5 shadow-2xl">
                    ...
                    >
                     ...
                     <DropdownMenuItem className="focus:bg-primary/10 focus:text-primary cursor-pointer rounded-lg py-2.5">
                        <Link href="/admin" className="flex w-full font-medium">Yönetim Paneli</Link>
                    </DropdownMenuItem>
