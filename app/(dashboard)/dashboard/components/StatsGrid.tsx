...
function StatCard({ title, value, icon, suffix, gradient, delay }: StatCardProps) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full justify-between">
        <div className="mb-4">
          <div className={`p-2.5 w-fit rounded-xl backdrop-blur-md mb-3 ${gradient}`}>
            {icon}
          </div>
        ...
        <div className="flex items-end gap-1">
          ...
          {suffix && (
            <span className="text-sm text-muted-foreground mb-1.5 font-medium">{suffix}</span>
          )}
        </div>
      </div>
    </motion.div>
  ...
  return (
    <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
      ...
      <StatCard
        ...
        value="85"
        suffix="/100"
        icon={<Award size={24} className="text-amber-500 dark:text-amber-400" />}
      ...
      <StatCard
        title="Üyelik Tipi"
        value="PRO"
        icon={<UserCheck size={24} className="text-purple-500 dark:text-purple-400" />}
