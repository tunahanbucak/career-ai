...
export default async function InterviewDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)]"> {/* Ekran boyutuna göre ayarlandı */}
      
      {/* Header */}
      <div className="flex-shrink-0 pb-6 border-b border-slate-800 mb-6">
        <Link href="/me/interviews" className="text-xs text-slate-500 hover:text-indigo-400 mb-2 inline-flex items-center gap-1 transition-colors">
             <ChevronLeft size={12} /> Tüm Mülakatlara Dön
        </Link>
        <div className="flex items-center justify-between">
            <div>
                <h1 className="text-2xl font-bold text-white">{interview.position}</h1>
                <p className="text-sm text-slate-400 mt-1 flex items-center gap-2">
                    <Bot size={14} className="text-emerald-400" /> 
                    AI Mülakat Simülasyonu 
                    <span className="text-slate-600">•</span>
                    <span className="flex items-center gap-1"><Calendar size={14}/> {new Date(interview.date).toLocaleDateString('tr-TR')}</span>
                </p>
            </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-6 rounded-2xl border border-slate-800 bg-slate-950/30 p-6 shadow-inner">
        ...
        {interview.messages.map((m) => {
          ...
          return (
            ...
            >
                {/* Avatar */}
                <div className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center ${isAI ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-indigo-500/10 text-indigo-400 border border-indigo-500/20"}`}>
